...
import { BudgetStatus } from "../types/notification";
...
import { createFinancialPlanner } from "./financialPlanner";
...
export function createScheduledTasks(deps: ScheduledTasksDeps) {
  const { userStore, expenseStore, budgetStore } = deps;

  const notificationService = createNotificationService(deps.notificationServiceDeps);
  const predictionEngine = createPredictionEngine({
    expenseStore,
    ...deps.predictionEngineDeps,
  });
  const financialPlanner = createFinancialPlanner({
    expenseStore,
    userStore,
    budgetStore,
  });
   ...
   */
  async function runDailyNotifications(): Promise<DailyNotificationResult> {
    const result: DailyNotificationResult = {
      usersProcessed: 0,
      billReminders: 0,
      budgetAlerts: 0,
      errors: [],
    ...
    for (const user of allUsers) {
      try {
        ...
        for (const bill of recurringExpenses) {
          if (shouldSendReminder(bill, now)) {
        ...
        if (budget && budget.totalBudget > 0) {
          ...
          const overBudgetCategories: ExpenseCategory[] = [];
          ...
          if (isBudgetAlertNeeded(budgetStatus)) {
   ...
   */
  async function runWeeklyTrendAnalysis(): Promise<WeeklyTrendResult> {
    const result: WeeklyTrendResult = {
      usersProcessed: 0,
      spikeWarnings: 0,
        ...
        for (const category of categories) {
          ...
          if (trendResult.data && trendResult.data.changePercentage > 20) {
            ...
            if (predictions.data) {
              ...
              if (categoryPrediction && isPredictedSpike(categoryPrediction)) {
