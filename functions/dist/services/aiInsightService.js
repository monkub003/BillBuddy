...
exports.createAiInsightService = createAiInsightService;
...
// ─── Service ────────────────────────────────────────────────
function createAiInsightService(deps) {
    ...
    function summarizeExpenses(expenses) {
        ...
        const lines = [`Total expenses: ฿${grandTotal.toLocaleString()}`];
...
Example: [{"type":"warning","title":"ค่าไฟอาจเพิ่มขึ้น","description":"จากแนวโน้มการใช้จ่ายที่สูงขึ้น","confidence":78}]`,
     ...
     */
    function generateRuleBasedInsights(expenses) {
        const insights = [];
        const grandTotal = expenses.reduce((s, e) => s + e.amount, 0);
        // Group by category
        const byCategory = new Map();
