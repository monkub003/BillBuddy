...
type ExtractionStep = "idle" | "uploading" | "extracting" | "done" | "error";
...
export function useExtraction() {
  ...
  const uploadImage = useCallback(
    async (uri: string): Promise<string | null> => {
      ...
      if (!userId) {
        setState((s) => ({ ...s, step: "error", error: "Not authenticated" }));
        return null;
      }
      ...
      try {
        ...
        setState((s) => ({ ...s, step: "error", loading: false, error: message }));
        return null;
      }
    },
    []
  ...
  const pollExtraction = useCallback(
    async (storageUrl: string): Promise<ExtractionResult | null> => {
  ...
  const extractFromImage = useCallback(
    async (uri: string): Promise<ExtractionResult | null> => {
